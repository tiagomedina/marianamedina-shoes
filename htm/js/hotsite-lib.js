var LibHotSite$=(function(){
  ...
  function fnSearchFil(sTxt,id){
    ...
    if(oFiltroItens){
      ...
      if(oTxtSearch){
        if(iQtdFound==0){oTxtSearch.innerHTML=FilNameAtt.replace("filter_","") +" n„o encontrado com o termo"}
  ...
  function fnRemoveAcento(str){
    var ComAc='·‡„‚‰ÈËÍÎÌÏÓÔÛÚıÙˆ˙˘˚¸Á¡¿√¬ƒ…» ÀÕÃŒœ”“’÷‘⁄Ÿ€‹«¥`^®~';  
  ...
  function fnHideFil(dif){
    //FunÁ„o para exibir/esconder filtro
  ...
  function fnChangeQtdProds(iQtd){
    ...
    if(oDivQtdProd){
      if(iQtd==0){oDivQtdProd.innerHTML="N„o foram encontrados produtos"}
